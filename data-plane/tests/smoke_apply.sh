...
if [ "$(id -u)" -ne 0 ]; then
	printf '%s\n' "apply smoke requires root/CAP_NET_ADMIN" >&2
	exit 1
fi
...
OUT_IF=aplout0
SINK_IF=aplsink0
LOG=${TMPDIR:-/tmp}/xdp-gateway-apply-smoke.$$.log
SNAPSHOT=${TMPDIR:-/tmp}/xdp-gateway-apply-smoke.$$.bin
PASS_SRC=${TMPDIR:-/tmp}/xdp-gateway-apply-pass.$$.bpf.c
PASS_OBJ=${TMPDIR:-/tmp}/xdp-gateway-apply-pass.$$.bpf.o
PASS_PIN=/sys/fs/bpf/xdp_gateway_apply_pass_$$
LOADER_PID=
...
if ! kill -0 "${LOADER_PID}" 2>/dev/null; then
	cat "${LOG}" >&2 || true
	printf '%s\n' "loader exited before apply smoke" >&2
	exit 1
fi
...
if [ "${before_slot}" != "0" ] || [ "${before_version}" != "1" ]; then
	printf '%s\n' "${before}" >&2
	printf '%s\n' "unexpected loader active_config before apply" >&2
	exit 1
fi

python3 "${TOOLS}" expect "${SRC_IF}" "${SINK_IF}" 45.45.0.1 10.0.0.2 deliver
python3 "${TOOLS}" generate-small "${SNAPSHOT}"
...
if [ "${after_slot}" != "1" ] || [ "${after_version}" != "2" ]; then
	printf '%s\n' "${after}" >&2
	printf '%s\n' "apply did not flip active_config exactly once" >&2
	exit 1
fi

python3 "${TOOLS}" expect "${SRC_IF}" "${SINK_IF}" 45.45.0.1 10.0.0.2 drop
python3 "${TOOLS}" expect "${SRC_IF}" "${SINK_IF}" 45.45.0.1 10.0.0.3 deliver
python3 "${TOOLS}" expect "${SRC_IF}" "${SINK_IF}" 45.45.0.66 10.0.0.3 drop
printf 'apply smoke: active_config flipped in %sms; service and feed verdicts verified\n' \
	"${elapsed_ms}"
