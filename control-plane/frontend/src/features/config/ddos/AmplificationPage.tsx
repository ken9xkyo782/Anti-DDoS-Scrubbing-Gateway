...
/**
 * Admin-only tab owning the one tunable DDoS vector: the UDP source ports the
 * node drops as reflection/amplification. Split out of DdosProtectionPage,
 * which now carries only the read-only protection-coverage overview.
 */
export function AmplificationPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', padding: 'var(--space-6)' }}>
      <PageHeader
        ...
        description="Attackers spoof your address and query open UDP services — DNS, NTP, SSDP, memcached — so the amplified reply floods you. The node drops any UDP packet whose source port is listed below. Built-in reflectors are compiled into the data plane and always on; the entries below them are yours to tune, node-wide."
        actions={
      ...
      <section style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
        <div>
          ...
          <p style={{ margin: 'var(--space-1) 0 0 0', fontSize: 'var(--font-size-sm)', color: 'var(--text-muted)' }}>
            Every UDP source port the node drops today. Changes reach the data plane within seconds —
            no rebuild and no service re-apply.
          </p>
        </div>
