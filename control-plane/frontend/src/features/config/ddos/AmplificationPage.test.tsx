...
import { AmplificationPage } from './AmplificationPage'
import {
...
describe('AmplificationPage & BlockedPortForm', () => {
  ...
  it('renders built-in and dynamic ports in one unified amplification table', () => {
    ...
    // Owns the page now — it is its own tab, not a section of DDoS Protection.
    expect(screen.getByText('UDP Reflection & Amplification')).toBeInTheDocument()
    expect(screen.getByText('Dynamic blocked source ports')).toBeInTheDocument()

    // Built-ins are rows, badged read-only and labelled with their reflector.
