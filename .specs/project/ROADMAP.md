...
**Telemetry & dashboards** - IN PROGRESS (spec + context + design + tasks drafted)
- End-to-end vertical slice: new **exact per-CPU per-service counters** on the XDP hot path (clean + per-reason drop pkts/bytes — D-TEL-1, clean-byte = billing source reused by chargeback) → worker `TELEMETRY_AGGREGATE` (new `JobType`+handler, in-worker ≤2 s scheduler) rolling windowed `TelemetryCounter(window_ts)` rows → telemetry/health API → **first React SPA** (shell + telemetry/node views only, D-TEL-2)
...
- Design `design.md` (**AD-030**) + 2 rendered diagrams (`diagrams/telemetry-architecture.{mmd,svg}` data-flow, `diagrams/aggregation-tick.{mmd,svg}` sequence). Key decisions: **D-030-1** `svc_stat_map` = prealloc `PERCPU_HASH[dp_id]` (288 B value, `drop_by_reason[DROP_REASON_CAP]` for ABI-stable width) counted at the `redirect_out`/`record_drop` choke points; **D-030-2** `pkt_meta.frame_len` (repurpose pad, stays 40 B) for byte counts; **D-030-4** new `ProtectedService.dp_id` (monotonic seq, ≥1) is the u32↔UUID surrogate written into `service_val.service_id` — **shared contract M4 #2 applier must adopt**; **D-030-5** reader = extend `dpstat` with `snapshot --json` (reuses `open_pinned_map`/`read_percpu_u64`); **D-030-6** aggregation = worker **background asyncio task** like the feed lane (NOT a Redis `JobType` — supersedes A-TEL-2, no ledger writes at 2 s); **D-030-7** node health = DP-derived fields persisted (`NodeHealthSnapshot`) + live `AgentJob`/`FeedSyncRun` reads; API reuses `load_service_for_principal` (404 cross-tenant) + `require_admin` + `/auth/me`; SPA = Vite+React+TS+TanStack Query (`refetchInterval` 2 s)+Recharts. New models `TelemetryCounter`+`NodeHealthSnapshot` (migration `20260710_0007`). **P1 core buildable now** against loader-seed/`FakeTelemetryReader`; true multi-service end-to-end gated on M4 #2 executed (`dp_id` contract)
- Tasks `tasks.md` (T1–T17; all 40 reqs mapped; 3 Design flags confirmed — no `TELEMETRY_AGGREGATE` JobType, `dp_id` in `create_service`, `drop_by_reason[32]`): **P1** T1 DP core (svc_stat+frame_len+wiring, dp-unit) · T2 loader pin+seed dp_id (dp-integration) · T3 `dpstat snapshot --json` (build+manual) · T4 `dp_id` model+seq+create_service+migration 0007 · T5 telemetry models+migration 0008 · T6 `TelemetryReader` `[P]` unit · T7 aggregator (delta/reset/prune) · T8 worker lane+settings · T9 API router · T10 SPA scaffold+auth `[P]` · T11 tenant dashboard · T12 admin dashboard · **P2** T13 top-talkers · T14 richer health · T15 P2 panels · **P3** T16 trend+export · T17 docs `[P]`. Cross-track parallel (DP ∥ CP ∥ FE); within CP only unit `[P]` (integration shares `compose.test`); FE separate toolchain (Vitest `fe` gate, new — T17 documents). All 3 pre-approval checks pass (granularity, diagram↔def, test co-location). Next: **Execute** (Phase 1: T1/T4 track leads, T6/T10 `[P]`)
