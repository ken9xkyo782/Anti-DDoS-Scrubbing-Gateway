...
**Telemetry & dashboards** - IN PROGRESS (P1 Execute reconciled 2026-07-13)
...
- P1 status: T1–T6 and T10 are complete; T5's final verification is `657cd3d` with 395 control-plane tests passing. The remaining P1 order is T7 → T9 → (T8 ∥ T11) → T12 → T17. T12 also makes FastAPI production SPA serving opt-in through `CONTROL_PLANE_FRONTEND_STATIC_DIR`, with an HTML-only history fallback that preserves API and asset 404s. P2 T13–T15 and P3 T16 remain deferred. Capture fresh control-plane and frontend baselines before T7; P1 remains in progress until final validation passes.
