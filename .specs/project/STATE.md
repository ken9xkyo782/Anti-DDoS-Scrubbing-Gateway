...
**Packet Parse Execute Status:** Complete and verified (2026-07-08). T1–T8 are committed; final gates passed: `cd data-plane && make bpf skel loader` and `cd data-plane && make test` → 21 passed. B-002 remains resolved in this environment. Resume = Service lookup & transparent redirect **Execute T1** (spec+context+design+tasks complete).
**Current Work:** M2 → **Drop-reason counters** — **spec + context + design complete** (`.specs/features/drop-reason-counters/`, DRC-01..17 all placed; D-DRC-1 per AD-016; design decisions AD-017; `design.md` + 2 rendered diagrams: drop-path flow + component/map layout). Scope: full §9.2 16-reason frozen index ABI (§9.2 doc order 0..15; one-move migration `map_error` 4→15, `bogon_drop` takes 4; `drop_reason_name[]` in header = source of truth; append-only ≥16 within `DROP_REASON_CAP=32`) + exact per-CPU counters (bad-reason clamps to `map_error`; reset-on-reload documented) + **ringbuf** drop-event sampling (256 KiB; per-CPU token bucket; runtime-tunable `sample_config`, defaults 256/s+burst 64 per CPU; `sample_stats` emitted/suppressed/lost; test determinism = `rate=0,burst=B`) + loader pinning `/sys/fs/bpf/xdp_gateway/` + `tools/dpstat` CLI (counters/tail/rate). New files: `src/drop_event.h`, `src/sample.h`, `tools/dpstat.c`; rewrites `drop_reason.h`; extends `loader.c`, `test_parse.c` (migration via enum symbols — no hardcoded indices), Makefile. `record_drop(meta, reason)` = single fused helper (DRC-15). Kernel semantics: ringbuf-in-XDP + NULL-on-full + `bpf_ktime_get_ns` web-verified; **test_run→ringbuf delivery = fail-fast de-risk case first in the test task** (fallback: dp-unit asserts `sample_stats` accounting only; event content in `make smoke`). Sequencing: after SLRD Execute. Out of scope: 9 M3 drop paths, per-service/billing counters, `bloom_hit_lpm_miss`, worker aggregation. **Design + Tasks APPROVED (2026-07-08).** Tasks (`tasks.md`, T1–T6, all 17 reqs mapped; pre-approval checks pass): **T1** ABI freeze (`drop_reason.h` rewrite + `drop_event.h` + DRC-04 case; quick, B+1) · **T2** `sample.h` + fused `record_drop(meta,r)` + call sites (quick, B+1 — sampling inert with zeroed config; assertions merge forward) · **T3** ringbuf **de-risk-first** + budget/content/fail-closed cases (quick, B+5; fallback = stats-only asserts + STATE Lessons entry) · **T4** loader pin+seed `[P]` (build) · **T5** `tools/dpstat` + Makefile target (build) · **T6** TESTING.md ABI table + README semantics (quick regression). Only T4 `[P]` (loader.c disjoint); T1→T2→T3 serialize on `drop_reason.h`/`xdp_gateway.bpf.c`/`test_parse.c`; baseline **B** = green `make test` count at T1 start. **Execute tooling (chosen):** Skill `coding-guidelines` T1–T5, none T6; no MCPs; **inline mode**. **Execute blocked on prerequisite: SLRD Execute first (D-DRC-1d).** Resume order = SLRD T1 → SLRD T7 → drop-reason-counters T1.
**Pending Execute (M2):** **Service lookup & transparent redirect** — **spec + context + design + tasks complete** (`.specs/features/service-lookup-redirect/`, SLRD-01..26 → T1–T7; D-SLRD-1..3; A-SLRD-1..8; design.md + 2 rendered diagrams). Gray areas AD-014; design decisions AD-015 (3 kernel semantics web-verified). **Tasks APPROVED (2026-07-08); Execute deferred** at user request (approve-only). **Execute tooling (chosen):** Skill `coding-guidelines` on C/XDP+shell T1–T6; MCPs none; **execution mode = inline** (like packet-parse). Resume = start T1. **Tasks:** **T1** contract headers (`service.h` new + `pkt_meta`/`drop_reason` ext; build) · **T2** config maps `service_map`=`ARRAY_OF_MAPS`[2]-of-`LPM_TRIE` + `active_config` + `tx_devmap` + **load de-risk** (map-in-map/LPM feasibility proven at load here; fallback = two named LPM maps + slot branch; quick, target 22) · **T3** service seam: slot-pin+LPM+`service_miss`/`disabled`/`map_error`+`bpf_redirect_map` + seed helpers + new service tests + **migrate clean-IPv4 tests** (`XDP_PASS`→`verdict==REDIRECT`; quick, target ~28) · **T4** ARP redirect seam + migrate ARP test (quick) · **T5** loader `OUT`+populate `tx_devmap` (fail-loud)+seed `[P]` (build) · **T6** live-veth `IN↔OUT` smoke = first **dp-integration** (real `XDP_REDIRECT`+TTL/csum; full) · **T7** TESTING.md dp-integration section. Only **T5** `[P]` (separate `loader/loader.c`, no tests); **T1→T2→T3→T4** serialize (shared `xdp_gateway.bpf.c`/`test_parse.c`); T6 not parallel-safe. Pre-approval checks (granularity / diagram-cross-check / test-co-location) all pass. Gates: build=`make bpf skel loader`, quick=`make test`, full=+`make smoke`. **Key risk front-loaded:** T2 proves/denies the map-in-map+LPM representation at load before any hot-path code. **Suite stays green:** clean-IPv4 (T3) + ARP (T4) terminals change `XDP_PASS`→redirect so their tests are migrated in-place (21 baseline → grows, no deletions). No `retval==XDP_REDIRECT` assertion in dp-unit (test-run can't produce it) — decision asserted via `test_meta_map.verdict`, real forward only in T6. Full design in `design.md` (AD-015); open items (→ Execute, fail-fast, not fabricated): map-in-map+LPM skeleton ergonomics (T2 gate; fallback ready), `tx_devmap` value type, smoke harness mechanism, CI kernel pin. Feature #2 of M2: the §8.2 clean-path terminal — establishes the config maps + slot-pin invariant all M3 config lookups reuse. No control-plane change. **Packet-parse prerequisite satisfied.** Resume = **Execute T1**.
**Prior M2 work:** **Packet parse & fail-fast** — **VERIFIED / executed** (`.specs/features/packet-parse/`, PKT-01..24 → T1–T8; D-PKT-1..4; A-PKT-1..7). Final gates: build=`make bpf skel loader`; quick=`make test` (`-DPKT_TEST_HOOKS`) → 21 passed. It bootstrapped `data-plane/` (`clang -target bpf` + libbpf skeleton), native/DRV loader on `IN` (fail-loud, no generic fallback), `BPF_PROG_TEST_RUN` synthetic-packet harness, shared `pkt_meta`/`drop_reason` contracts, minimal per-CPU `counter_map`, data-plane TESTING.md conventions, L2/VLAN/QinQ EtherType resolution, IPv4+L4 single parse, the four fail-fast drops, ARP `XDP_PASS` seam, and valid IPv4 `XDP_PASS` service-lookup seam. This satisfies the prerequisite for **Service lookup & transparent redirect**. Open non-blocking follow-ups remain: CI kernel/toolchain pin for `BPF_PROG_TEST_RUN`, privileged live-veth smoke, and later replacement of the ARP/service seams by the redirect feature.
...
**Impact:** M2 Drop-reason counters `design.md` (all 17 DRC reqs placed) + 2 rendered diagrams. New `src/drop_event.h` + `src/sample.h` + `tools/dpstat.c`; rewrites `drop_reason.h` (frozen 0..15 ABI per AD-016 + `drop_reason_name[]` + static-assert headroom); extends `loader.c` (pin+seed), `test_parse.c` (symbol-based migration + sampling cases), Makefile. Establishes the bpffs-pinning pattern the M4 worker will reuse for map access. Next: **Tasks**.
...
**Impact:** M2 Service lookup & redirect `design.md` (all 26 SLRD reqs placed) + 2 rendered diagrams. Modifies the executed `data-plane/` (`xdp_gateway.bpf.c`, `pkt_meta.h`, `drop_reason.h`, `loader.c`, `Makefile`, `test_parse.c`) and **migrates the 21 packet-parse tests** whose `XDP_PASS` terminal changed. Establishes the config-map/slot-pin/`tx_devmap` patterns + the first `dp-integration` (`make smoke`) convention (extends `TESTING.md`, A-SLRD-8). Next: **Tasks**.
