...
**Current Work:** M1 → **Apply-status state machine** — spec + context + design + **tasks** complete (`.specs/features/apply-status/`, APLY-01..40 → T1–T7; D-APLY-1..3; A-APLY-1..6). Awaiting approval → Execute. Owns the `pending→queued→applying→active|failed` machine behind one guard, the API auto-enqueue (`pending→queued` + real Redis + `AgentJob` ledger, idempotent by version), the worker-facing `mark_applying/active/failed` (version-guarded, "no stale-over-new"), and the per-service apply-status read API (9.2) + admin job-list. Design adds `agent_job` table (`UNIQUE(target_type,target_id,version)` + `ON DELETE CASCADE`), pure `core/applystate.py` guard (`assert_transition` + `LEGAL_APPLY` + monotonic `active_version`), `services/apply.py` (enqueue outbox + `mark_*` + retry + reads + `ApplyDispatcher`), one router, one Alembic rev; **transactional outbox** (job written in mutation txn, post-commit best-effort `LPUSH apply:jobs`) + **version guard under the same `protected_service` FOR UPDATE lock** `bump_version` takes. Modifies service-rule-list's service/rule/list services + `bump_version`. Diagrams rendered (component + state-machine + enqueue-apply sequence). Tasks: **T1** guard (unit, `[P]`) · **T2** `agent_job` model+migration · **T3** enqueue outbox+dispatcher+reads · **T4** version-guarded `mark_*`+retry · **T5** apply-status/retry/jobs router · **T6** wire enqueue into SRL services+`bump_version` · **T7** SRL routers→202 (all 40 reqs mapped; only T1 `[P]`). Requires service-rule-list executed first; adds enqueue-only Redis + `AgentJob` model (worker loop = M4).
