...
**Status**: Verified — T1–T11 complete
...
**Tools**: Bash, Write/Edit · Skill: `coding-guidelines`
**Done when**:
- [x] `create_rule` persists + `bump_version` + audit (SRL-15/21/34); duplicate priority → 409 (SRL-16); 17th rule → `RuleLimitError` 409 (SRL-17); invalid port range → 422 (SRL-19)
- [x] Overlapping rule → success + `warnings[]` naming the overlapped rule(s) (SRL-18); `overlap_dry_run` reports overlaps without writing (SRL-37)
- [x] Concurrent inserts at the same priority → exactly one succeeds (SRL-38); concurrent 16→17 under the row-lock → exactly one 409 (SRL-17)
- [x] `list/get/update/delete` scoped to the owning service (SRL-20)
- [x] Gate check passes: `ruff check . && ruff format --check . && mypy app/ && pytest -q` (full)
...
**Tools**: Bash, Write/Edit · Skill: `coding-guidelines`
**Done when**:
- [x] `add_whitelist` accepts arbitrary external IPv4 (`198.51.100.7/32`, `45.0.0.0/8`), rejects IPv6/host-bits → 422; source NOT required ⊆ allocation (SRL-22/23); `bump_version` + audit (SRL-34)
- [x] `add_blacklist(scope='service')` on owned service persists + `bump_version` + audit (SRL-26); IPv6 → 422 (SRL-27); the same source may sit in both whitelist and blacklist with no conflict (SRL-41)
- [x] `add_blacklist(scope='global')` persists `service_id=NULL`, `source='manual'`; list returns `source` discriminator (SRL-28/30)
- [x] `list/remove` scoped (service entries → owning service; global → admin) (SRL-25)
- [x] Gate check passes: `ruff check . && ruff format --check . && mypy app/ && pytest -q` (full)
- [x] Test count: ≥9 tests pass (no silent deletions)
**Tests**: integration
**Gate**: full
...
**Tools**: Bash, Write/Edit · Skill: `coding-guidelines`
**Done when**:
- [x] Revoking an `AllocatedCIDR` that contains an active `ProtectedService` → 409 naming the blocking service(s) (SRL-32)
- [x] After the service is deleted (T3), revoking the same CIDR → succeeds (tenant-cidr TCA-15/17 still hold)
- [x] No import cycle: `python -c "import app.main"` succeeds
- [x] Gate check passes: `ruff check . && ruff format --check . && mypy app/ && pytest -q` (full)
...
**Tools**: Bash, Write/Edit · Skill: `coding-guidelines`
**Done when**:
- [x] Owner (or admin) loads their service; `tenant_user` requesting another tenant's service id → 404, zero leak (SRL-31)
- [x] Unknown service id → 404
- [x] Gate check passes: `ruff check . && ruff format --check . && mypy app/ && pytest -q` (full)
- [x] Test count: ≥3 tests pass (no silent deletions)
**Tests**: integration
**Gate**: full
...
**Tools**: Bash, Write/Edit · Skill: `coding-guidelines`
**Done when**:
- [x] Create inside allocation → 201 (`pending`, `version=1`); outside → 403; overlap → 409; IPv6/host-bits `cidr_or_ip` → 422 (SRL-01/04/08/40); `committed>ceiling` → 422 (SRL-03)
- [x] `tenant_user` `PATCH /plan` → 403; admin sizes plan, oversubscription → 200 + warning (SRL-07/36); list annotates owner for admin, scopes for tenant_user (SRL-05)
- [x] `disable` (confirm) → dangerous audit, `pending`; idempotent repeat no-ops (SRL-09/11); enabling a **zero-rule** service → 200, no error (SRL-39)
- [x] Delete enabled → 409; disable then delete → 204 + children cascade + audit (SRL-12/13/14); editing dest to overlap → 409 (SRL-44)
- [x] Cross-tenant service access → 404 zero-leak (SRL-31)
- [x] Gate check passes: `ruff check . && ruff format --check . && mypy app/ && pytest -q` (full)
...
**Tools**: Bash, Write/Edit · Skill: `coding-guidelines`
**Done when**:
- [x] Admin `POST /blacklist` → 201 (`source=manual`, `service_id=NULL`) + audit (SRL-28); list/delete work, `source` discriminator present (SRL-30)
- [x] `tenant_user` on any `/blacklist` endpoint → 403, no side effect (SRL-29)
- [x] Gate check passes: `ruff check . && ruff format --check . && mypy app/ && pytest -q` (full)
